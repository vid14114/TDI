...
public class Wallpaper {
	...
	public BufferedImage markArea(ArrayList<TDI> colorAreas) {
		...
		for (int i = 0; i < colorAreas.size(); i++) {
			...
			int selectedPosX = colorAreas.get(i).getIcons().get(0)
					.getPosition().y;
			int selectedPosY = colorAreas.get(i).getIcons().get(0)
					.getPosition().x;
			...
			if (colorAreas.get(i).isLocked()) {
				...
				g2.fill(new Rectangle2D.Float(colorAreas.get(i).getIcons()
						.get(0).getPosition().x
						* scalingX, colorAreas.get(i).getIcons().get(0)
						.getPosition().y
						* scalingY, 10 * scalingX, 10 * scalingY));
			}
