...
/**
 * Implements Runnable interface. Is Master, is big. rotation(z,x,y) [annahme]
 * Info: rechts neigen => x alt > x neu links neigen => x alt < x neu oben
 * neigen => y alt > y neu runter neigen => y alt < y neu rechts drehen => z alt
 * > z neu links drehen => z alt < z neu
 * <p>
 * Position(x,y,z) heben => z neu > (z alt - var) //e.g. var = 200; [not used]
 * rechts bewegen: x alt > x neu links bewegen: x alt < x neu oben bewegen: y
 * alt > y neu unten bewegen: y alt < y neu
 */
public class BigLogic implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals("Restore")) {
			...
			if ((checkIp(tdiDialog.getIp1().getText(), tdiDialog.getIp2()
			...
			new Runnable() {
				@Override
				public void run() {
					final String[] plugins = new String[pluginTableModel
							.getRowCount()];
			...
			final Timer mo = new Timer();
			mo.scheduleAtFixedRate(new TimerTask() {

				@Override
				public void run() {
					final ArrayList<TDI> tdis = server.fullPose();
					if (tdis != null) {
						for (final TDI t : tdis) {
							newCommand(t);
						}
					}
				}
