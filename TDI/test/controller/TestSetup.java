...
public class TestSetup {
	...
	public static void aSetup() throws InterruptedException {
		...
		final TDI t1 = new TDI((byte) 49, 100, 100, 100,
				new float[] { 0, 0, 0 });
		b = new BigLogic();
		tdis.add(t1);
		b.setTdis(tdis);
		b.getTdis().add(
				new TDI((byte) 50, 200, 200, 200, new float[] { 0, 0, 0 }));
		b.splitIcons();
		final Tilt tilt = new Tilt(new float[] { 0, 0, 0 });
		tilt.setTiltListener(new TiltHandler(b));

		final Rotation rotation = new Rotation(tdis);
		rotation.setRotationListener(new RotationHandler(b));

		final Move move = new Move(tdis);
	...
	public void bRotateRightDesktop() throws InterruptedException {
		...
		final TDI t2 = new TDI((byte) 49, 100, 100, 100,
				new float[] { 40, 0, 0 });
	...
	public void cRotateLeftDesktop() throws InterruptedException {
		...
		final TDI t2 = new TDI((byte) 49, 100, 100, 100, new float[] { -40, 0,
				0 });
		b.newCommand(t2);
		Thread.sleep(5000);
	}
	...
	public void dLockUnlock() throws InterruptedException {
		...
		b.newCommand(new TDI((byte) 49, 100, 100, 100,
				new float[] { 0, 0, -50 }));
	...
	public void eCloseProgram() throws InterruptedException {
		...
		// Start with test
		final int openedPrograms = ProgramHandler.getRunningPrograms().size();
		// 1. Lock TDI
		b.newCommand(new TDI((byte) 49, 100, 100, 100,
				new float[] { 0, 0, -60 }));
		b.newCommand(new TDI((byte) 49, 100, 100, 100, new float[] { 0, 0, 0 }));
		...
		Assert.assertEquals((openedPrograms + 1), ProgramHandler
				.getRunningPrograms().size());
		Assert.assertEquals(TDIState.inapp, b.getTdis().get(1).getState());
		// 4. Get the TDI out of in app mode, by tilting right
		b.newCommand(new TDI((byte) 50, 200, 200, 200,
				new float[] { 0, 0, -70 }));
		...
		b.newCommand(new TDI((byte) 50, 200, 200, 200, new float[] { 0, 0, 0 }));
		Assert.assertEquals(openedPrograms, ProgramHandler.getRunningPrograms()
				.size());
		Thread.sleep(1000);
	}
	...
	public void zTutorial1() throws InterruptedException {
		aSetup(); // begin the tutorial from scratch, nix gut two threads
					// running
		System.out.println("doubleRight");
		final TDI t1 = new TDI((byte) 49, 100, 100, 100,
				new float[] { 0, 0, 50 });
		final TDI t2 = new TDI((byte) 49, 100, 100, 100, new float[] { 0, 0,
				100 });
		b.newCommand(t1);
		b.newCommand(t2);
	}
	...
	@Ignore
	// Need other tests before this one, pls do :)
	// @Test
	public void zTutorial8() {
		System.out.println("minimize");
		b.getTdis().get(0).setState(TDIState.window);
		final TDI t1 = new TDI((byte) 49, 100, 100, 100, new float[] { -45, 0,
				0 });
		final TDI t2 = new TDI((byte) 49, 100, 100, 100,
				new float[] { 0, 0, 0 });
		final TDI t3 = new TDI((byte) 49, 100, 100, 100, new float[] { -45, 0,
				0 });
		b.newCommand(t1);
		b.newCommand(t2);
		b.newCommand(t3);
