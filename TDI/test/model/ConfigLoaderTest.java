...
public class ConfigLoaderTest {
	...
	public void prepareEnvironment() throws IOException {
		 ...
		 * to a backup folder in TDI This can then be used for the test
		 */
	...
	// BufferedImage shouldWallpaper = ImageIO.read(new
	// File(Executor.getBackground())); //Insert your wallpaper path here -->
	// xfce
	...
	// Assert.assertEquals(shouldWallpaper, ImageIO.read(new
	// File(TDIDirectories.TDI_RESTORE).listFiles()[0]));
	...
	public void testLoadIcons() throws IOException {
		...
		Runtime.getRuntime().exec(
				new String[] { "cp", "-r",
						"resources/ConfigLoaderTest/desktop1/*",
						System.getProperty("user.home") + "/Desktop/" });
		icons = cf.loadIcons();

		// Asserting for each icon in the file
		Assert.assertEquals(15, icons.size());
		Assert.assertSame("xdg-open ~",
				icons.get(icons.indexOf(new Icon("Home", null))).getExecPath());
		Assert.assertSame("xdg-open trash:///",
				icons.get(icons.indexOf(new Icon("Rubbish Bin", null)))
						.getExecPath());
		...
		Runtime.getRuntime().exec(
				new String[] { "cp", "-r",
						"resources/ConfigLoaderTest/desktop2/*",
						System.getProperty("user.home") + "/Desktop/" });
		icons = cf.loadIcons();
		...
		Runtime.getRuntime().exec(
				new String[] { "cp", "-r",
						"resources/ConfigLoaderTest/desktop3/*",
						System.getProperty("user.home") + "/Desktop/" });
		icons = cf.loadIcons();
