...
public class ConfigLoaderTest {
	...
	public void testLoadWallpaper() throws IOException {
		final BufferedImage shouldWallpaper = ImageIO.read(new File(Executor
	...
	public void testPluginMethods() throws IOException {
		...
		Assert.assertEquals("lo.jar", res[0][0]);
		Assert.assertEquals(false, res[0][1]);
		int falses = 0;
		for (final Object[] plugin : res)
			if (!(boolean) plugin[1]) {
				falses++;
			}
		...
		final String[] savedPlugins = new String[4];
		for (int i = 0; i < 4; i++) {
		...
		Assert.assertEquals(5, res.length);
		Assert.assertEquals("lo.jar", res[0][0]);
		Assert.assertEquals(true, res[0][1]);
		int trues = 0;
		for (final Object[] plugin : res)
			if ((boolean) plugin[1]) {
				trues++;
